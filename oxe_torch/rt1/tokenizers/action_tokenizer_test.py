...
from oxe_torch.rt1.tokenizers.action_tokenizer import RT1ActionTokenizer
import numpy as np
...
class ActionTokenizerTest(unittest.TestCase):
    ...
    def testTokenizeAndDetokenizeIsEqual(self):
        action_space_dict = OrderedDict([('done', spaces.Discrete(1)), 
                                         ('xyz', spaces.Box(low= -1.0, high= 1.0, shape=(3,), dtype=np.float32)),
                                         ('rpy', spaces.Box(low= -np.pi / 2., high= np.pi / 2., shape=(3,), dtype=np.float32)),
                                         ('grasp', spaces.Box(low= 0  , high= 1.0, shape=(1,), dtype=np.float32))
                                         ])
