...
from torchvision.models.maxvit import MBConv
...
class Attention(nn.Module):
    def __init__(self, dim, dim_head=32, dropout=0.0, window_size=7, bias=False):
        super().__init__()
        assert (
            dim % dim_head
        ...
        self.to_out = nn.Sequential(
            nn.Linear(dim, dim, bias=bias), nn.Dropout(dropout)
        )
...
class ViTConfig:
    """Configuration class to store the configuration of a `MaxViTEncoder`."""
...
class MaxViTEncoder(nn.Module):
    def __init__(
        ...
        in_channels = [config.stem_channels_in] + config.block_channel_ins[:-1]
        out_channels = config.block_channel_ins
        ...
        net = maxvit_t(weights=MaxVit_T_Weights.DEFAULT if config.pretrained else None)
        self.stem = net.stem
    ...
    def forward(
        ...
        cond_drop_prob: Optional[float] = 0.1,
        cond_fns: Optional[Tuple[Callable, ...]] = None,
        return_embeddings: bool = True,
    ):
        x = self.stem(x)
        if torch.any(torch.isnan(x)):
            print("x is nan after stem")
        cond_fns = iter(default(cond_fns, []))
        cond_fn = next(cond_fns, None)
        if torch.any(torch.isnan(x)):
            print("x is nan after first cond fn")
        if exists(cond_fn):
            x = cond_fn(x)
        cond_fn = next(cond_fns, None)
...
class TransformerAttention(nn.Module):
    def __init__(
        ...
        self.to_kv = nn.Linear(dim_context, dim_head * 2, bias=bias)
        self.to_out = nn.Sequential(
                ...
                nn.ModuleList(
                    [
                        TransformerAttention(
                            dim=dim, dim_head=dim_head, heads=heads, dropout=attn_dropout, bias=bias, causal_attention=causal_attention
                        ),
...
class RT1(nn.Module):
    def __init__(
        ...
        self.action_mean = config.action_mean
    ...
    def forward(
        ...
        attended_tokens = self.transformer(
            ...
            attn_mask=~attn_mask if exists(attn_mask) else None,
        )
...
class RTX1(nn.Module):
    ...
    def __init__(
        ...
        self.vit = MaxViTEncoder(vit_config)
        self.model = RT1(
            config=rt1_config,
            vit=self.vit,
        )
